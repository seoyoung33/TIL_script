...
//객체.속성
//사용자정의함수 없이 속성 및 내장함수(alert,prompt,typeof...)를 작성하면 사용자 동작과 관계없이 즉시 실행된다.
console.log(`초기값 키 : ${userHeight.value}`); //즉시실행
console.log(`초기값 몸무게 : ${userWeight.value}`);
//사용자가 키를 입력한 후 프로그램이 그 값을 인식하게 만드는 법
//값을 실시간으로 입력하는 동안은 프로그램 과부하 및 보안 문제로 함수를 작동하게 해선 안되며 결정버튼을 따로 만들어두고 버튼에 특정동작을 해야만 함수가 실행되게 만들어야 한다. =>리뷰같은건 가능함! 무겁지않으니까!
...
function weightCal(){
    ...
    const overWeight = userWeight.value-normalWeight;
    console.log(overWeight);
    //결과예) 당신의 적정체중은 ?kg이며 ?kg 초과되셨습니다.
    console.log(`당신의 적정체중은 ${normalWeight}kg이며 ${overWeight}kg 초과되셨습니다.`);
    //input 요소일 경우 값 읽기/수정/삭제는 value 속성 사용
    //input이 아닌 다른 요소일 경우 값 읽기/수정/삭제 textContent 또는 innerHTML 속성 사용
    result.innerHTML = `당신의 적정체중은 <em>${normalWeight}kg</em>이며<br><em>${overWeight}kg</em> 초과되셨습니다.`
}