...
btn1.addEventListener('click',()=>{
    //window.scrollTo(0,500) //가로스크롤 없음, 세로스크롤 200 (숫자는 자동으로 px로 반환된다.)
    window.scrollTo({ //추가 속성이나 메서드 연결 시에는 x y를 {객체} 문법으로 작성한다.
        left:0,
        top:4000,
        behavior:'smooth' //맨 위로 버튼을 할수도 있겠다!!! top을 0으로 바꾸면 됩니덩
    })
})
//behavior:'smooth'
...
const toonNext = document.querySelector('.toon_btn .next');
const toonPrev = document.querySelector('.toon_btn .prev');
